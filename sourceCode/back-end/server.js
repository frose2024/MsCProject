...
connectDB()
  ...
  .catch((error) => {
    ...
    process.exit(1);  // Exits process if the database connection fails
  });
...
app.use((req, res, next) => {
  // Debug: console.log(`Incoming request: ${req.method} ${req.originalUrl}`);
  next();
});

app.use('/uploads', (req, res, next) => {
  Debug: console.log('Static file request:', req.url);
  next();
