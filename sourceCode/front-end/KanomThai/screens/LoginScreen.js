...
const decodeJWT = (token) => {
  try {
    const base64Url = token.split('.')[1]; // Extract payload from the JWT
...
export default function LoginScreen({ navigation, setIsLoggedIn, setIsAdmin, setUserId }) {
  const [identifier, setIdentifier] = useState(''); // User's identifier (username/email)
  ...
  const handleLogin = async () => {
    try {
      ...
      // Save credentials locally using AsyncStorage
